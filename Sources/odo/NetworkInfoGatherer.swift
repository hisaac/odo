...
enum NetworkInfoGatherer {
	static func gatherNetworkInfo() throws -> NetworkInfo {
		...
		guard getifaddrs(&ifap) == 0 else {
			throw NSError(
				domain: "SystemInfoError",
				code: 2,
				userInfo: [NSLocalizedDescriptionKey: "Failed to get network interfaces"]
			)
		}
		...
		while current != nil {
			...
			if let sockaddr = addr.ifa_addr {
				switch Int32(sockaddr.pointee.sa_family) { case AF_INET:
					...
					if getnameinfo(
						...
						interface.ipv4Addresses.append(
							String(
								decoding: hostname.prefix(while: { $0 != 0 })
					...
					case AF_INET6:
						...
						if getnameinfo(
							...
							interface.ipv6Addresses.append(
								String(
									decoding: hostname.prefix(while: { $0 != 0 })
										.map { UInt8(bitPattern: $0) },
									as: UTF8.self
					...
					case AF_LINK:
						let linkAddr = sockaddr.withMemoryRebound(to: sockaddr_dl.self, capacity: 1)
						{ $0.pointee }
						if linkAddr.sdl_alen == 6 {  // MAC address length
							let macBytes = withUnsafePointer(to: linkAddr) { ptr in
								ptr.withMemoryRebound(
									to: UInt8.self,
									capacity: MemoryLayout<sockaddr_dl>.size
								) { bytes in
									Array(
										UnsafeBufferPointer(
											start: bytes.advanced(
												by: Int(linkAddr.sdl_nlen) + Int(linkAddr.sdl_alen)
													- 6
											),
											count: 6
										)
